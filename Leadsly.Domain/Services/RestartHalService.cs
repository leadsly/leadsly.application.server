...
{
    public class RestartHalService : IRestartHalService
    {
        public RestartHalService(
            IAwsElasticContainerService awsEcsService,
            IVirtualAssistantRepository virtualAssistantRepository,
            ILogger<RestartHalService> logger,
        ...
        private readonly ILogger<RestartHalService> _logger;

        public async Task RestartAsync(string halId, CancellationToken ct = default)
        {
            _logger.LogDebug("Restarting Hal Id {halId}", halId);
