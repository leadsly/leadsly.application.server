...
{
    public class FollowUpMessagesCreateMQService : IFollowUpMessagesCreateMQService
    {
        public FollowUpMessagesCreateMQService(
            ILogger<FollowUpMessagesCreateMQService> logger,
            IFollowUpMessageCreateMQService service,
        ...
        private ILogger<FollowUpMessagesCreateMQService> _logger;
        private IFollowUpMessageCreateMQService _service;
        ...
        public async Task<IList<CampaignProspectFollowUpMessage>> GenerateProspectsFollowUpMessagesAsync(string halId, CancellationToken ct = default)
        {
            ...
            return await GenerateMessagesAsync(halId, activeCampaigns);
        }

        public async Task<IList<CampaignProspectFollowUpMessage>> GenerateProspectsFollowUpMessagesAsync(string halId, IList<Campaign> activeCampaigns, CancellationToken ct = default)
        {
            _logger.LogDebug("HalId {halId} has {activeCampaignsCount} active campaigns.", halId, activeCampaigns?.Count);

            return await GenerateMessagesAsync(halId, activeCampaigns, ct);
        }

        private async Task<IList<CampaignProspectFollowUpMessage>> GenerateMessagesAsync(string halId, IList<Campaign> activeCampaigns, CancellationToken ct = default)
        {
